"""
Analytical query engine (DuckDB locally, Neon Postgres in production).

Storage is selected by RANKINGS_STORE. SQL stays parameterized; the adapter
in ranking_store.py rewrites placeholders and catalog probes.
"""
...
from backend.data.ranking_store import (
    bump_local_generation,
    execute_sql,
    get_store_conn,
)
...
def _get_conn():
    """Return the serving-store connection (DuckDB or Postgres)."""
    return get_store_conn()


def get_serving_conn():
...
def _execute(sql: str, params: list[Any] | None = None, *, context: str) -> list[dict[str, Any]]:
    """Run `sql` against the configured serving store."""
    return execute_sql(sql, params, context=context)
...
def invalidate_view(position: str, weekly: bool = False) -> None:
    """Bump in-process cache generation after a local bake or publish."""
    bump_local_generation()
